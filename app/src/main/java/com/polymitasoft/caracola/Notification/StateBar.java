...
public class StateBar {
    ...
    public void notificar(Context context_emisor, Class<CaracolaApplication> context_receptor, String titulo, String text, String info, String ticker, String nota) {
        ...
        NotificationCompat.Builder mBuilder = (NotificationCompat.Builder) new NotificationCompat.Builder(context_emisor).setSmallIcon(android.R.drawable.sym_def_app_icon).setContentTitle(titulo).setContentText(text).setContentInfo(info).setTicker(ticker);
...
//        PendingIntent piDismiss = PendingIntent.getActivity(context_emisor, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        mBuilder.setStyle(new NotificationCompat.BigTextStyle().bigText(nota)).addAction(android.R.drawable.sym_def_app_icon, "Confirmar", null).setAutoCancel(true);
